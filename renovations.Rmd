...
This is a bit tricky, and we can't just compare sales (say) the week before the shutdown to the week of the shutdown because perhaps there are some seasonal effects (eg people buy more coffee around Valentines day) that need to be accounted for.
...
Up until the competitor shut down, the difference between what we predict and what actually happened is zero. But then we see a change after the competitor shuts down. We show the cumulative effect, i.e. the total amount of sales estimated as a result of their shutting down, and we see that by the end of their shutdown the coffee shop has have made an additional 12k or so over what we would have expected to make if they had stayed open.

The prediction intervals (in gray) show the range of likely values.  At the end of the shutdown our best single estimate is that we made an additional 12k, but that could actually be anywhere between 8k and 15k. (All good statistical results will provide a range of values, not just a single figure, so that you can assess alternate scenarios)
...
In summary, while the competitor shut down the coffee shop made more money than would have been expected otherwise.
...
In this time window it looks like there was an overall cumulative increase in sales of approximately 8k by the end of the data period in mid-October. But it is also interesting in that there is an initial decline in the cumulative sales - perhaps customers (both the coffee shop's, and the competitor's who came to us when their regular shop was shut down) decided to give the renovated shop a try. And then the sales pick up again (perhaps people liked the coffee shop's coffee even better than the renovated competitor's) until about Sept 1, when the sales are back to what would have expected. And (at least for the next month or so) the sales are larger than would have been expected. 
...
Analyzing the sales data helped the coffee shop understand the impact of an external event on their sales, allowing them to better estimate future sales.
